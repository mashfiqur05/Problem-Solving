...
int query(int node, int left, int right, int findVal) {
    if (segm_tree[node] < findVal) return -1;
    if (left == right) return left;
    int mid = left + (right - left)/2;
    if (segm_tree[2*node] >= findVal) return query (2*node, left, mid, findVal);
    else return query (2*node+1, mid+1, right, findVal);
}
...
int32_t main() {
    ...
    while (q--) {
        ...
        if (type == 1) {
            int val; cin >> val;
            a++;
            update(1, 1, n, a, val); // no -1 needed now
        } else {
